...
CONFIG = {
    ...
    "OUTPUT_DIR": "./training_results_rl_compare",
    ...
    "WEIGHT_CONFIGS": [
        {"name": "W80E20", "w_td": 0.8, "w_eq": 0.2, "desc": "需求优先"},
        {"name": "W70E30", "w_td": 0.7, "w_eq": 0.3, "desc": "需求较优先"},
        {"name": "W60E40", "w_td": 0.6, "w_eq": 0.4, "desc": "平衡偏需求"},
    ...
    # GA算法和MP算法参数
    "GA_POP_SIZE": 100,  # GA 种群大小
    "GA_GENERATIONS": 500,  # GA 演化代数
    "MP_MAX_NODES": 1000,  # MP 算法搜索的子图节点上限
...
class HybridStateEncoder:
    ...
    def _sample_candidates(self, current_pos, sequence, maps, direction_hint):
        ...
        for dr in range(-radius, radius + 1):
            for dc in range(-radius, radius + 1):
                ...
                if not (0 <= nr < self.H and 0 <= nc < self.W):
                    continue
                if maps['forbidden'][nr, nc] == 1.0 or (nr, nc) in sequence:
                    continue
                ...
                if len(sequence) >= (CONFIG["MAX_STEPS"] // 2):
                    sr, sc = sequence[0]
        ...
        if len(candidates) > CONFIG["MAX_CANDIDATES"] and len(sequence) >= 2:
    ...
    def _encode_candidates(self, maps, sequence, current_pos, existing_net, budget_used, direction_hint):
        ...
        for dr in range(-radius, radius + 1):
            for dc in range(-radius, radius + 1):
                ...
                if maps['forbidden'][nr, nc] == 1.0 or (nr, nc) in sequence: continue
                dist = math.sqrt(dr ** 2 + dc ** 2)
                if not (min_d <= dist <= max_d): continue

                if target_dr != 0.0 or target_dc != 0.0:
                    if (dr * target_dr + dc * target_dc) / (dist + 1e-9) < hard_cos_threshold: continue
                ...
                if len(sequence) >= 2:
                    ...
                    if n1 > 0 and n2 > 0 and (v1[0] * v2[0] + v1[1] * v2[1]) / (n1 * n2) < CONFIG["MIN_TURN_ANGLE_COS"]:
                        continue
...
class MetroExpansionEnv(gym.Env):
    ...
    def step(self, action_idx):
        ...
        done = (self.steps >= CONFIG["MAX_STEPS"]) or (
                self.budget_used >= CONFIG["TOTAL_BUDGET"]) or self.force_terminate
...
class SpatialCNN(nn.Module):
    def __init__(self, in_ch=6, channels=[32, 64, 128]):
        ...
        for idx, ch in enumerate(channels):
            ...
            if idx == len(channels) - 1: layers.append(SelfAttention(ch))
            curr_ch = ch
...
class CandidateLSTM(nn.Module):
    def __init__(self, in_dim=5, hidden_dim=256):
        super().__init__()
        self.mlp, self.lstm = nn.Sequential(nn.Linear(in_dim, hidden_dim), nn.ReLU()), nn.LSTM(hidden_dim, hidden_dim,
                                                                                               batch_first=True)

    def forward(self, x, mask):
        out, _ = self.lstm(self.mlp(x))
...
class HybridActorCritic(nn.Module):
    ...
    def forward(self, sp, cand, mask):
        ...
        if mask is not None and mask.sum(dim=1).min() == 0:
...
class PPOAgent:
    ...
    def select_action(self, obs):
        ...
        if cand.size(1) == 0:
            with torch.no_grad(): _, val = self.policy_old(sp, torch.zeros((1, 1, 5), device=CONFIG["DEVICE"]),
                                                           torch.zeros((1, 1), dtype=torch.bool,
                                                                       device=CONFIG["DEVICE"]))
            return -1, 0.0, val.item()
        with torch.no_grad():
            logits, val = self.policy_old(sp, cand, mask)
...
class DQNAgent:
    def __init__(self):
        ...
        self.epsilon, self.epsilon_decay, self.epsilon_min = 1.0, 0.999, 0.05
...
def evaluate_full_route_baseline(route, reward_calc, existing_net, dir_hint):
    ...
    if route:
        reward_calc.reset_episode(route[0], existing_net)  # ✅ 必须加
...
# ==========================================
# 9. Baseline 2: MP 算法 (严格整数线性规划 ILP - Gurobi)
# 需要安装: pip install gurobipy
# 并且需要配置有效的 Gurobi License
# ==========================================
...
class MP_Runner:
    ...
    def run(self):
        ...
        print(
            f"[{get_timestamp()}] ⚠️ 提示: 为保证 ILP 的计算可解性，算法将基于硬约束构建受限候选子图，并调用 Gurobi 求解器。")
        ...
        try:
            ...
            # 步骤 3: 求解与路径解析 (Solving & Parsing)
            print(f"[{get_timestamp()}] 调用 Gurobi 求解器 (限时 60 秒)......")
            m.optimize()
            print(f"\n[{get_timestamp()}] ---------- Gurobi 求解统计信息 ----------")
            print(f"[{get_timestamp()}] ⚙️ 变量总数 (NumVars):    {m.NumVars}")
            print(f"[{get_timestamp()}] ⚙️ 线性约束数 (NumConstrs): {m.NumConstrs}")
            print(f"[{get_timestamp()}] ⚙️ 求解耗时 (Runtime):      {m.Runtime:.3f} 秒")
            try:
                print(f"[{get_timestamp()}] ⚙️ 探索节点数 (NodeCount):  {int(m.NodeCount)}")
            except AttributeError:
                pass

            if m.Status == GRB.OPTIMAL or m.Status == GRB.TIME_LIMIT or m.Status == GRB.SUBOPTIMAL:
                if m.SolCount > 0:
                    print(f"[{get_timestamp()}] 🎯 最终目标值 (ObjVal):   {m.ObjVal:.4f}")
                    print(f"[{get_timestamp()}] 🎯 最好界 (ObjBound):     {m.ObjBound:.4f}")
                    try:
                        print(f"[{get_timestamp()}] 🎯 相对 Gap (MIPGap):     {m.MIPGap * 100:.2f}%")
                    except AttributeError:
                        print(f"[{get_timestamp()}] 🎯 相对 Gap (MIPGap):     N/A (可能未定义或完美最优)")
                    ...
                    print(f"[{get_timestamp()}] ------------------------------------------------\n")
                    print(f"[{get_timestamp()}] ⚠️ 警告: Gurobi 达到时间限制但未找到可行解，返回初始单点。")
                    return [self.start_pos]
            else:
                print(f"[{get_timestamp()}] ------------------------------------------------\n")
                print(f"[{get_timestamp()}] ⚠️ 警告: Gurobi 求解失败，状态码: {m.Status}，返回初始单点。")
                return [self.start_pos]

        except gp.GurobiError as e:
            print(f"[{get_timestamp()}] ❌ Gurobi Error: {e}")
            return [self.start_pos]
        except Exception as e:
            print(f"[{get_timestamp()}] ❌ 发生未知错误: {e}")
            return [self.start_pos]


# ==========================================
# 10. Network Evaluator (服务能力与脆弱性评估)
# ==========================================
...
class ExperimentRunner:
    ...
    def run_ppo(self, start_pos, dir_hint, w_td, w_eq, weight_name):
        ...
        for ep in range(1, CONFIG["TOTAL_EPISODES"] + 1):
            ...
            if ep % 50 == 0:
                ...
                print(f"[{get_timestamp()}] PPO[{weight_name}] epoch {ep:4d} | Rtotal {ep_r:7.2f} | "
        ...
        self._plot_single_alg_curves(history, "PPO", weight_name)
    ...
    def _plot_single_alg_curves(self, history, alg_name, weight_name):
        ...
        plt.plot(eps, history['rewards'], color='royalblue', alpha=0.3, label='Average reward')
        trend = gaussian_filter1d(history['rewards'], sigma=100)
        plt.plot(eps, trend, color='darkorange', linewidth=2, label='Trend')
        plt.xlabel('Episode', fontsize=12)
        plt.ylabel('Average reward', fontsize=12)
        plt.title(f'{alg_name} Training Reward Performance ({weight_name})', fontsize=14)
        plt.legend()
        plt.grid(True, linestyle='--', alpha=0.5)
        plt.savefig(os.path.join(output_dir, f"Fig9_{alg_name}_Reward_{weight_name}.png"), dpi=300)
        plt.close()
        fig, ax1 = plt.subplots(figsize=(10, 6))

        if alg_name == "DQN":
            color_loss = 'crimson'
            ax1.plot(eps, history['losses'], color=color_loss, alpha=0.8, label='Q-Network loss')
            ax1.set_xlabel('Episode', fontsize=12)
            ax1.set_ylabel('Loss Value', fontsize=12, color=color_loss)
            ax1.tick_params(axis='y', labelcolor=color_loss)
            ax1.grid(True, linestyle='--', alpha=0.5)
            ax1.legend(loc='upper right')
            plt.title(f'{alg_name} Loss Convergence ({weight_name})', fontsize=14)
        else:
            color_critic = 'crimson'
            line1 = ax1.plot(eps, history['critic_losses'], color=color_critic, alpha=0.8, label='Critic loss')
            ax1.set_xlabel('Episode', fontsize=12)
            ax1.set_ylabel('Critic Loss Value', fontsize=12, color=color_critic)
            ax1.tick_params(axis='y', labelcolor=color_critic)
            ax1.grid(True, linestyle='--', alpha=0.5)

            ax2 = ax1.twinx()
            color_actor = 'forestgreen'
            line2 = ax2.plot(eps, history['actor_losses'], color=color_actor, alpha=0.8, label='Actor loss')
            ax2.set_ylabel('Actor Loss Value', fontsize=12, color=color_actor)
            ax2.tick_params(axis='y', labelcolor=color_actor)

            lines = line1 + line2
            labels = [l.get_label() for l in lines]
            ax1.legend(lines, labels, loc='upper right')
            plt.title(f'{alg_name} Actor-Critic Loss Convergence ({weight_name})', fontsize=14)

        fig.tight_layout()
        plt.savefig(os.path.join(output_dir, f"Fig10_{alg_name}_Loss_{weight_name}.png"), dpi=300)
        plt.close()
    ...
    def _plot_combined_rewards(self, histories_dict, weight_name):
        ...
        for alg, hist in histories_dict.items():
            ...
            trend = gaussian_filter1d(rewards_raw, sigma=100)
            plt.plot(eps, trend, color=colors[alg], linewidth=2.5, label=f'{alg} Trend')

        plt.xlabel('Episode', fontsize=12)
        plt.ylabel('Average reward', fontsize=12)
        plt.title(f'RL Algorithms Reward Comparison ({weight_name})', fontsize=14)
        plt.legend(loc='lower right')
        plt.grid(True, linestyle='--', alpha=0.5)
        plt.savefig(os.path.join(CONFIG["OUTPUT_DIR"], f"Fig_Combined_Reward_{weight_name}.png"), dpi=300)
        plt.close()
        print(f"[{get_timestamp()}] 📊 本轮权重 {weight_name} 下的 7 张训练曲线图已全部生成并保存。")
    ...
    def run_single_weight_config(self, line_cfg: Dict, weight_cfg: Dict) -> Dict[str, Any]:
        ...
        self._plot_combined_rewards({"PPO": hist_ppo, "DQN": hist_dqn, "DDPG": hist_ddpg}, w_name)
        # 2. 运行启发式算法 GA
        ga_runner = GA_Runner(self.encoder, reward_calc, start_pos, self.global_net, dir_hint)
        route_ga = ga_runner.run()
        # 3. 运行精确算法 MP
        try:
            mp_runner = MP_Runner(self.encoder, reward_calc, start_pos, self.global_net, dir_hint)
            route_mp = mp_runner.run()
        except Exception as e:
            print(f"[WARN] MP skipped or failed: {e}")
            route_mp = [start_pos]
        results = {
            "weight_name": w_name, "w_td": w_td, "w_eq": w_eq,
            "PPO": {"route": route_ppo, "source": "PPO_Core"},
            "DQN": {"route": route_dqn, "source": "DQN_Base"},
            "DDPG": {"route": route_ddpg, "source": "DDPG_Base"},
            "GA": {"route": route_ga, "source": "GA_Heuristic"},
            "MP": {"route": route_mp, "source": "MP_Exact"}
        }
            ...
            for w_res in line_res['weight_results']:
                for alg in ['PPO', 'DQN', 'DDPG', 'GA', 'MP']:
                    summary_rows.append({
                        ...
                        'w_eq': w_res['w_eq'],
                        'Algorithm': alg, 'Rtotal': w_res[alg]['rtotal'], 'Rtd': w_res[alg]['rtd'],
                        'Req': w_res[alg]['req'],
                        'Len': w_res[alg]['len'], 'Transfers': w_res[alg]['transfers'], 'Cost': w_res[alg]['cost'],
                        'Budget_OK': w_res[alg]['budget_ok'],
                        'Service_Cap': w_res[alg]['service_cap'],
                        'TEL_Vul': w_res[alg]['tel_vul']
                    })
        return summary_rows

    def run_all(self):
        print(f"\n[{get_timestamp()}] 🔥 启动算法实验")
        all_results = []
        ...
        summary_rows = self._print_final_summary(all_results)
        return summary_rows
    ...
    def _save_route_files(self, sequence, name: str, weight_name: str = "", w_td: float = None,
        ...
        for idx, (r, c) in enumerate(sequence):
            ...
            row_data = {"station_id": idx, "grid_r": r, "grid_c": c, "lon": lon, "lat": lat}
            if weight_name:
                row_data.update({"weight_config": weight_name, "w_td": w_td, "w_eq": w_eq})
            if budget_used is not None:
                row_data.update(
                    {"budget_used": budget_used, "budget_limit": CONFIG["TOTAL_BUDGET"], "budget_ok": budget_ok})
                    ...
                    for st in stations:
                        if st in self.proc.station_to_grid:
                            ...
                            if s_lon != 0.0 and s_lat != 0.0:
                                folium.CircleMarker(
                                    location=[s_lat, s_lon], radius=2.5, color=line_color,
                                    weight=1.5, fill=True, fill_color=line_color, fill_opacity=0.9, popup=line_name
                                ).add_to(m_obj)

                route_color, fill_color = '#808080', '#cccccc'
                if "PPO" in name:
                    route_color, fill_color = '#ff5733', '#ff8c00'
                folium.PolyLine(folium_coords, color=route_color, weight=6, opacity=0.9).add_to(m_obj)
                for i, (lat, lon) in enumerate(folium_coords):
                    folium.CircleMarker(
                        location=[lat, lon], radius=5, color='white', weight=1.5, fill=True,
                        fill_color=fill_color, fill_opacity=1.0, popup=f'Station {i}'
                    ).add_to(m_obj)
            m_bg = folium.Map(location=center, zoom_start=11, tiles='cartodbpositron')
            draw_folium_base(m_bg)
            m_bg.save(os.path.join(CONFIG["OUTPUT_DIR"], f"{name}_map_with_bg.html"))
            m_nobg = folium.Map(location=center, zoom_start=11, tiles=None)  # tiles=None 去除底图
        ...
        def draw_matplotlib_img(show_grid=True):
            ...
            if show_grid:
                ...
                ax.grid(which='minor', color='#00bfff', linestyle='-', linewidth=0.3)
                ax.set_xlabel("Grid X (Columns)", fontsize=12)
                ax.set_ylabel("Grid Y (Rows)", fontsize=12)
            ...
            for line_name, stations in CONFIG["PRESET_LINES"].items():
                ...
                if r_coords:
                    ...
                    ax.plot(c_coords, r_coords, color='red', marker='.', markersize=6, linestyle='-',
            ...
            if sequence:
                ...
                for r, c in sequence:
                    gen_r.append(r)
                    gen_c.append(c)
                ax.plot(gen_c, gen_r, color='green', marker='.', markersize=7,
                        linestyle='-', linewidth=2.0, label='Generated Route')

            if show_grid:
                ax.legend(loc='upper right', fontsize=12)

            suffix = "_grid_map" if show_grid else "_pure_network"
            plt.savefig(os.path.join(CONFIG["OUTPUT_DIR"], f"{name}{suffix}.png"), dpi=300, bbox_inches='tight',
...
# ==========================================
def aggregate_seed_results(all_seeds_data: List[Dict], seeds: List[int]):
    print(f"\n[{get_timestamp()}] 📊 开始自动汇总 {len(seeds)} 个随机种子的实验结果...")

    if not all_seeds_data:
        return print(f"[{get_timestamp()}] ❌ 未找到数据。")
    df_all = pd.DataFrame(all_seeds_data)

    summary_stats = df_all.groupby(['Line', 'Weight', 'Algorithm']).agg(
        Rtotal_Mean=('Rtotal', 'mean'), Rtotal_Std=('Rtotal', 'std'),
        Rtd_Mean=('Rtd', 'mean'), Req_Mean=('Req', 'mean'),
        Cost_Mean=('Cost', 'mean'), Success_Count=('Budget_OK', 'sum'),
        Service_Mean=('Service_Cap', 'mean'), Service_Std=('Service_Cap', 'std'),
        TEL_Mean=('TEL_Vul', 'mean'), TEL_Std=('TEL_Vul', 'std')
    ).reset_index()
    summary_stats['Rtotal_Std'] = summary_stats['Rtotal_Std'].fillna(0.0).round(2)
    summary_stats['Service_Std'] = summary_stats['Service_Std'].fillna(0.0).round(2)
    summary_stats['TEL_Std'] = summary_stats['TEL_Std'].fillna(0.0).round(4)
    print(f"[{get_timestamp()}] 🎉 汇总计算完成！\n{summary_stats.head(10).to_string()}")
...
if __name__ == "__main__":
    ...
    for seed in test_seeds:
        ...
        with open(log_path, "w", encoding="utf-8") as f_log:
            ...
            try:
                ...
                seed_summary = runner.run_all()
                for row in seed_summary:
                    row['Seed'] = seed
                all_seeds_data.extend(seed_summary)
    ...
    aggregate_seed_results(all_seeds_data, test_seeds)
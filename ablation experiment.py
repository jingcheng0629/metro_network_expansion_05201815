...
CONFIG = {
    ...
    "OUTPUT_DIR": "./training_results_rl_compare_NoGlobalState_ActionMasking",
    ...
    "WEIGHT_CONFIGS": [
        {"name": "W70E30", "w_td": 0.7, "w_eq": 0.3, "desc": "需求较优先"},
    ],
...
CONFIG["ABLATION_CONFIGS"] = [
    {"name": "Full_PPO"},
    {"name": "No_GlobalStateModule",
     "use_global_state_module": False,
     "use_six_channel_encoder": False,
     "use_spatial_cnn": False,
     "use_attention": False,
     "use_candidate_sequence_module": False,
     "use_feature_coupling": False},
    {"name": "No_ActionMasking", "use_geometric_candidate_rules": True,
     "use_action_masking": False, "use_budget_constraint": True},
    {"name": "No_PPOUpdate", "use_ppo_update": False},
    {"name": "No_SixChannel", "use_six_channel_encoder": False},
    {"name": "No_SpatialCNN", "use_spatial_cnn": False},
    {"name": "No_Attention", "use_attention": False},
    {"name": "No_CandidateSequence", "use_candidate_sequence_module": False},
    {"name": "No_FeatureCoupling", "use_feature_coupling": False},
]
CONFIG["CURRENT_ABLATION"] = DEFAULT_ABLATION.copy()
CONFIG["RUN_ABLATION_NAMES"] = [
    "Full_PPO",
    "No_ActionMasking",
    "No_PPOUpdate",
    "No_SixChannel",
    "No_SpatialCNN",
    "No_Attention",
    "No_CandidateSequence",
    "No_FeatureCoupling",
    "No_GlobalStateModule",
]
...
def release_runtime_memory():
    """Release Python and CUDA allocator state after the ablation run."""
    gc.collect()
    if torch.cuda.is_available():
...
def get_ablation_config() -> Dict[str, Any]:
    cfg = DEFAULT_ABLATION.copy()
    cfg.update(CONFIG.get("CURRENT_ABLATION", {}))
    return cfg


def get_ablation_name() -> str:
    return str(get_ablation_config().get("name", "Full_PPO"))
...
class HybridStateEncoder:
    ...
    def _sample_candidates(self, current_pos, sequence, maps, direction_hint):
        ...
        use_rules = get_ablation_config().get("use_geometric_candidate_rules", True)
    ...
    def encode(self, maps, sequence, current_pos, existing_net, budget_used, direction_hint):
        ...
        if not ablation.get("use_six_channel_encoder", True):
            spatial = maps['poi'].unsqueeze(0)
        elif not ablation.get("use_global_state_module", True):
    ...
    def _encode_candidates(self, maps, sequence, current_pos, existing_net, budget_used, direction_hint):
        ...
        for r, c in candidates:
            ...
            if get_ablation_config().get("use_action_masking", True):
...
class MetroExpansionEnv(gym.Env):
    ...
    def step(self, action_idx):
        ...
        use_action_masking = get_ablation_config().get("use_action_masking", True)
        use_budget_constraint = get_ablation_config().get("use_budget_constraint", True)
...
class SpatialCNN(nn.Module):
    def __init__(self, in_ch=6, channels=[32, 64, 128]):
        super().__init__()
        self.use_spatial_cnn = get_ablation_config().get("use_spatial_cnn", True)
        self.use_attention = get_ablation_config().get("use_attention", True)
...
class CandidateLSTM(nn.Module):
    def __init__(self, in_dim=5, hidden_dim=256):
        super().__init__()
        self.use_sequence_module = get_ablation_config().get("use_candidate_sequence_module", True)
...
class HybridActorCritic(nn.Module):
    def __init__(self):
        super().__init__()
        self.ablation = get_ablation_config()
        self.use_global_state_module = self.ablation.get("use_global_state_module", True)
        self.use_spatial_cnn = self.ablation.get("use_spatial_cnn", True)
        self.use_feature_coupling = self.ablation.get("use_feature_coupling", True)
        self.use_action_masking = self.ablation.get("use_action_masking", True)
        self.use_spatial_branch = self.use_global_state_module
        spatial_channels = 6 if self.ablation.get("use_six_channel_encoder", True) else 1
...
class PPOAgent:
    def __init__(self):
        self.use_ppo_update = get_ablation_config().get("use_ppo_update", True)
...
class ExperimentRunner:
    ...
    def run_ppo(self, start_pos, dir_hint, w_td, w_eq, weight_name):
        ablation_name = get_ablation_name()
        ablation_tag = make_file_tag(ablation_name)
        print(f"[{get_timestamp()}] Start PPO training | weight={weight_name} | ablation={ablation_name}")
        ...
        for ep in range(1, CONFIG["TOTAL_EPISODES"] + 1):
            ...
            if ep % 50 == 0:
                ...
                print(f"[{get_timestamp()}] PPO[{weight_name}/{ablation_name}] epoch {ep:4d} | Rtotal {ep_r:7.2f} | "
        ...
        self._plot_single_alg_curves(history, f"PPO_{ablation_tag}", weight_name)
    ...
    def _plot_ablation_reward_comparison(self, histories_dict, weight_name, line_name):
        merged_histories = dict(histories_dict or {})

        if not merged_histories:
            print(f"[{get_timestamp()}] No ablation histories were collected; skip ablation reward comparison plot.")
            return

        output_dir = CONFIG["OUTPUT_DIR"]
        color_map = {
            "Full_PPO": "#d95f02",
            "No_GlobalStateModule": "#1b9e77",
            "No_ActionMasking": "#7570b3",
            "No_PPOUpdate": "#e7298a",
            "No_SixChannel": "#66a61e",
            "No_SpatialCNN": "#e6ab02",
            "No_Attention": "#a6761d",
            "No_CandidateSequence": "#1f78b4",
            "No_FeatureCoupling": "#b2df8a",
        }

        plt.figure(figsize=(13, 8))
        for variant_name, hist in merged_histories.items():
            eps = np.asarray(hist.get("episodes", []), dtype=float)
            rewards_raw = np.asarray(hist.get("rewards", []), dtype=float)
            if len(eps) == 0 or len(rewards_raw) == 0:
                continue

            color = color_map.get(variant_name, "dimgray")
            sigma = min(100, max(1, len(rewards_raw) // 10))
            trend = gaussian_filter1d(rewards_raw, sigma=sigma)

            plt.plot(eps, rewards_raw, color=color, linewidth=0.7, alpha=0.08)
            plt.plot(eps, trend, color=color, linewidth=3.2, label=variant_name)

        plt.xlabel('Episode', fontsize=18)
        plt.ylabel('Average reward', fontsize=18)
        plt.title(f'PPO Ablation Reward Comparison ({line_name}, {weight_name})', fontsize=18)
        plt.tick_params(axis='both', labelsize=15)
        plt.legend(loc='lower right', frameon=True, fontsize=15)
        plt.grid(True, linestyle='--', alpha=0.45)
        plt.tight_layout()

        line_tag = make_file_tag(line_name)
        fig_path = os.path.join(output_dir, f"Fig_Ablation_Reward_Comparison_{line_tag}_{weight_name}.png")
        plt.savefig(fig_path, dpi=600, bbox_inches='tight')
        plt.close()

        print(f"[{get_timestamp()}] Ablation reward comparison figure saved to: {fig_path}")
    ...
    def run_single_weight_config_ablation(self, line_cfg: Dict, weight_cfg: Dict,
                                          baseline_results: Dict[str, Any] = None) -> Dict[str, Any]:
        w_name, w_td, w_eq, w_desc = weight_cfg['name'], weight_cfg['w_td'], weight_cfg['w_eq'], weight_cfg['desc']
        print(
            f"\n{'=' * 80}\n[{get_timestamp()}] >>> PPO ablation config: {w_name} ({w_desc}): w_td={w_td}, w_eq={w_eq}\n{'=' * 80}")
        start_pos, dir_hint = self.proc.get_grid_by_name(line_cfg['start']), line_cfg['direction']
        reward_calc = RewardCalculatorASOC(self.proc.maps, w_td, w_eq)
        evaluator = NetworkEvaluator(self.proc)
        ...
        results = {
            "weight_name": w_name, "w_td": w_td, "w_eq": w_eq,
            "ablation_results": [],
            "PPO": {"route": [], "source": "PPO_Full_PPO"},
        }
        ablation_histories = {}

        print(
            f"{'-' * 140}\n| {'Variant':<20} | {'Rtotal':<9} | {'Rtd':<8} | {'Req':<8} | {'Len':<4} | {'Tr':<3} | {'Budget':<8} | {'Service':<9} | {'TEL(Vul)':<8} | {'Status':<8} |\n{'-' * 140}")

        run_names = CONFIG.get("RUN_ABLATION_NAMES")
        if run_names is None:
            selected_ablation_configs = CONFIG["ABLATION_CONFIGS"]
        else:
            run_name_set = set(run_names)
            selected_ablation_configs = [cfg for cfg in CONFIG["ABLATION_CONFIGS"] if cfg["name"] in run_name_set]

        for ablation_cfg in selected_ablation_configs:
            current_ablation = DEFAULT_ABLATION.copy()
            current_ablation.update(ablation_cfg)
            CONFIG["CURRENT_ABLATION"] = current_ablation

            variant_name = current_ablation["name"]
            variant_tag = make_file_tag(variant_name)
            if variant_name == "Full_PPO" and baseline_results is not None:
                route_ppo = baseline_results["PPO"]["route"]
                hist_ppo = baseline_results["training_histories"]["PPO"]
                print(f"[{get_timestamp()}] Reusing the Full_PPO run from the algorithm comparison.")
            else:
                set_seed(CONFIG["SEED"])
                route_ppo, _, hist_ppo = self.run_ppo(start_pos, dir_hint, w_td, w_eq, w_name)
            ablation_histories[variant_name] = hist_ppo

            rt, td, eq, cost, tr = evaluate_full_route_baseline(route_ppo, reward_calc, self.global_net, dir_hint)
            budget_ok = cost <= CONFIG["TOTAL_BUDGET"]
            if len(route_ppo) > 1:
                G_net, interchange_sts = evaluator._build_full_network(route_ppo)
                service_cap, station_loads = evaluator.eval_service_capacity(G_net, interchange_sts)
                tel_vul = evaluator.eval_vulnerability(G_net, station_loads, alpha=0.3, num_attacks=3)
            else:
                service_cap, tel_vul = 0.0, 1.0
            ...
            variant_result = {
                "variant": variant_name, "source": f"PPO_{variant_tag}", "route": route_ppo,
                "rtotal": rt, "rtd": td, "req": eq, "cost": cost, "transfers": tr,
                "len": len(route_ppo), "budget_ok": budget_ok,
                "service_cap": service_cap, "tel_vul": tel_vul
            }
            results["ablation_results"].append(variant_result)

            if variant_name == "Full_PPO":
                results["PPO"].update(variant_result)

            print(
                f"| {variant_name:<20} | {rt:9.2f} | {td:8.2f} | {eq:8.2f} | {len(route_ppo):4d} | {tr:3d} | {cost:8.2f} | {service_cap:9.1f} | {tel_vul:8.3f} | {'OK' if budget_ok else 'OVER':<8} |")
            self._save_route_files(route_ppo, f"{line_cfg['name']}_PPO_{variant_tag}_{w_name}", w_name, w_td, w_eq,
                                   cost, budget_ok)

        self._plot_ablation_reward_comparison(ablation_histories, w_name, line_cfg['name'])
        self._save_ablation_route_panel(line_cfg['name'], results["ablation_results"], w_name, w_td, w_eq)

        CONFIG["CURRENT_ABLATION"] = DEFAULT_ABLATION.copy()
        print(f"{'-' * 140}")
        return results

    def _print_cross_weight_summary_legacy(self, line_name: str, line_results: List[Dict]):
        print(f"\n{'=' * 90}\n[{get_timestamp()}] 📊 线路 [{line_name}] 跨权重配置总结 (全算法 Rtotal 对比)\n{'=' * 90}")
        print(f"{'Weight':<10} | {'PPO':<9} | {'DQN':<9} | {'DDPG':<9} | {'GA':<9} | {'MP':<9}\n{'-' * 90}")

        for res in line_results:
            print(
                f"{res['weight_name']:<10} | {res['PPO']['rtotal']:9.2f} | {res['DQN']['rtotal']:9.2f} | {res['DDPG']['rtotal']:9.2f} | {res['GA']['rtotal']:9.2f} | {res['MP']['rtotal']:9.2f}")
        print(f"{'-' * 90}")

    def _print_cross_weight_summary(self, line_name: str, line_results: List[Dict]):
        print(f"\n{'=' * 120}")
        print(f"[{get_timestamp()}] Line [{line_name}] PPO ablation summary (actual metric values)")
        print(f"{'=' * 120}")

        header = (
            f"{'Line':<10} | {'Weight':<10} | {'Variant':<22} | {'Rtotal':>9} | {'Rtd':>8} | {'Req':>8} | "
            f"{'Len':>4} | {'Tr':>3} | {'Cost':>8} | {'Service':>9} | {'TEL(Vul)':>8} | {'Budget':>8}"
        )
        print(header)
        print("-" * len(header))

        for res in line_results:
            for variant in res.get("ablation_results", []):
                budget_label = "OK" if variant.get("budget_ok", False) else "OVER"
                print(
                    f"{line_name:<10} | {res['weight_name']:<10} | {variant['variant']:<22} | "
                    f"{variant['rtotal']:9.2f} | {variant['rtd']:8.2f} | {variant['req']:8.2f} | "
                    f"{variant['len']:4d} | {variant['transfers']:3d} | {variant['cost']:8.2f} | "
                    f"{variant['service_cap']:9.1f} | {variant['tel_vul']:8.3f} | {budget_label:>8}"
                )
        print(f"{'=' * 120}")

    def _print_final_summary(self, all_results: List[Dict]):
        print(f"\n{'#' * 80}\n[{get_timestamp()}] 🏆 最终综合总结报告 (全算法)\n{'#' * 80}")
        summary_rows = []
        for line_res in all_results:
            for w_res in line_res['weight_results']:
                for variant in w_res.get("ablation_results", []):
                    summary_rows.append({
                        'Line': line_res['line'], 'Weight': w_res['weight_name'], 'w_td': w_res['w_td'],
                        'w_eq': w_res['w_eq'],
                        'Algorithm': 'PPO', 'Ablation': variant['variant'],
                        'Rtotal': variant['rtotal'], 'Rtd': variant['rtd'], 'Req': variant['req'],
                        'Len': variant['len'], 'Transfers': variant['transfers'], 'Cost': variant['cost'],
                        'Budget_OK': variant['budget_ok'],
                        'Service_Cap': variant['service_cap'],
                        'TEL_Vul': variant['tel_vul']
                    })
        if summary_rows:
            summary_df = pd.DataFrame(summary_rows)
            print(f"\n[{get_timestamp()}] Final in-memory summary (no CSV written):")
            print(summary_df.to_string(index=False))
        else:
            print(f"\n[{get_timestamp()}] No summary rows were produced.")
    ...
    def run_all(self):
        print(f"\n[{get_timestamp()}] 🔥 启动单算法实验 - 全局方向绝对硬约束版")
        all_results = []
        for line_cfg in CONFIG["LINE_CONFIG"]:
            print(
                f"\n{'#' * 80}\n[{get_timestamp()}] >>> 线路: {line_cfg['name']} (起点: {line_cfg['start']}, 预期方向: {line_cfg['direction']})\n{'#' * 80}")
            line_results = []
            for w_cfg in CONFIG["WEIGHT_CONFIGS"]:
                baseline = self.run_single_weight_config(line_cfg, w_cfg)
                ablation = self.run_single_weight_config_ablation(line_cfg, w_cfg, baseline)
                ablation["algorithm_results"] = baseline
                line_results.append(ablation)
            all_results.append({"line": line_cfg['name'], "weight_results": line_results})
            self._print_cross_weight_summary(line_cfg['name'], line_results)
        self._print_final_summary(all_results)

    def _save_ablation_route_panel(self, line_name: str, variant_results: List[Dict], weight_name: str,
                                   w_td: float = None, w_eq: float = None):
        if not variant_results:
            return

        run_names = CONFIG.get("RUN_ABLATION_NAMES")
        if run_names is None:
            variant_order = [cfg["name"] for cfg in CONFIG["ABLATION_CONFIGS"]]
        else:
            variant_order = [name for name in run_names]
        variant_map = {res["variant"]: res for res in variant_results}
        ordered_results = [variant_map[name] for name in variant_order if name in variant_map]
        if not ordered_results:
            return

        all_r_coords, all_c_coords = [], []
        existing_lines = []
        for preset_name, stations in CONFIG["PRESET_LINES"].items():
            r_coords, c_coords = [], []
            for st in stations:
                if st in self.proc.station_to_grid:
            ...
            if r_coords:
                existing_lines.append((preset_name, r_coords, c_coords))

        for res in ordered_results:
            for r, c in res.get("route", []):
                all_r_coords.append(r)
                all_c_coords.append(c)
        ...
        n_variants = len(ordered_results)
        ncols = 3 if n_variants > 4 else 2
        ...
        full_rtotal = variant_map.get("Full_PPO", {}).get("rtotal", np.nan)

        for ax_idx, ax in enumerate(axes):
            if ax_idx >= len(ordered_results):
                ax.axis('off')
                continue

            res = ordered_results[ax_idx]
            ...
            ax.grid(which='major', color='#00bfff', linestyle='-', linewidth=0.8)
            ax.grid(which='minor', color='#00bfff', linestyle='-', linewidth=0.25)
            ax.tick_params(axis='both', labelsize=12)

            existing_plotted = False
            for _, r_coords, c_coords in existing_lines:
                label = 'Existing network' if not existing_plotted else None
                base_color, edge_color, mid_color = '#1f77b4', '#0d4a75', '#73b3e6'
                ax.plot(c_coords, r_coords, color=base_color, marker='o', markersize=5.0,
                        linestyle='-', linewidth=1.5, markerfacecolor=base_color,
                        markeredgecolor=edge_color, markeredgewidth=0.8, zorder=2, label=label)
                ax.plot(c_coords, r_coords, linestyle='none', marker='o', markersize=3.0,
                        markerfacecolor=mid_color, markeredgecolor='none', zorder=3)
                ax.plot(c_coords, r_coords, linestyle='none', marker='o', markersize=1.0,
                        markerfacecolor='white', markeredgecolor='none', zorder=4)
                existing_plotted = True

            route = res.get("route", [])
            if route:
                gen_r = [p[0] for p in route]
                gen_c = [p[1] for p in route]
                route_color, edge_color, mid_color = self._get_route_style_colors(line_name)
                ax.plot(gen_c, gen_r, color=route_color, marker='o', markersize=6.0,
                        linestyle='-', linewidth=1.8, markerfacecolor=route_color,
                        markeredgecolor=edge_color, markeredgewidth=0.8, zorder=5, label='Generated route')
                ax.plot(gen_c, gen_r, linestyle='none', marker='o', markersize=3.6,
                        markerfacecolor=mid_color, markeredgecolor='none', zorder=6)
                ax.plot(gen_c, gen_r, linestyle='none', marker='o', markersize=1.2,
                        markerfacecolor='white', markeredgecolor='none', zorder=7)

            rtotal = res.get("rtotal", np.nan)
            delta = rtotal - full_rtotal if not pd.isna(rtotal) and not pd.isna(full_rtotal) else np.nan
            delta_text = f", delta={delta:+.2f}" if not pd.isna(delta) and variant_name != "Full_PPO" else ""
            ax.set_title(f"{variant_name}\nRtotal={rtotal:.2f}{delta_text}", fontsize=16)

            x_span, y_span = max_c - min_c, max_r - min_r
            scale_len_km = 5
            x0 = min_c + 0.06 * x_span
            y0 = min_r + 0.07 * y_span
            ax.plot([x0, x0 + scale_len_km], [y0, y0], color='black', linewidth=3.2, solid_capstyle='butt')
            ax.text(x0 + scale_len_km / 2, y0 + 0.025 * y_span, "5 km",
        ...
        for ax_idx, ax in enumerate(axes):
            if ax_idx >= n_variants:
                continue
            row_idx = ax_idx // ncols
            col_idx = ax_idx % ncols
            if row_idx == nrows - 1:
                ax.set_xlabel("Grid X (1 km cells)", fontsize=15)
            if col_idx == 0:
                ax.set_ylabel("Grid Y (1 km cells)", fontsize=15)

        fig.suptitle(
            f"PPO Ablation Route Comparison - {line_name} / {weight_name}\n"
            f"Spatial reference: grid coordinates, 1 km x 1 km cells (mapped from WGS84 lon/lat)",
            fontsize=18
        )
        fig.tight_layout(rect=[0, 0, 1, 0.94])

        output_png = os.path.join(CONFIG["OUTPUT_DIR"],
                                  f"Fig6_AblationRoutes_{make_file_tag(line_name)}_{make_file_tag(weight_name)}.png")
        fig.savefig(output_png, dpi=600, bbox_inches='tight')
        plt.close(fig)
        print(f"[{get_timestamp()}] Ablation route comparison figure saved to: {output_png}")
...
if __name__ == "__main__":
    ...
    for seed in test_seeds:
        print(f"\n{'=' * 80}\n🚀 开始执行随机种子 Seed = {seed} 的独立实验...\n{'=' * 80}\n")
        CONFIG["SEED"] = seed;
        set_seed(seed)
        seed_output_dir = os.path.join(base_output_dir, f"seed_{seed}");
        CONFIG["OUTPUT_DIR"] = seed_output_dir;
        ensure_dir(seed_output_dir)
        log_path = os.path.join(seed_output_dir,
                                f"experiment_log_seed{seed}_{get_timestamp().replace(':', '-').replace(' ', '_')}.txt")
        with open(log_path, "w", encoding="utf-8") as f_log:
            # Use the active stream so this also works when executed in Jupyter.
            original_stdout = sys.stdout;
            sys.stdout = _Tee(original_stdout, f_log)
            try:
                print(f"[{get_timestamp()}] 📄 已开启日志: {log_path}")
    ...
    print(f"\n{'=' * 80}\n[{get_timestamp()}] 🎉 所有 {len(test_seeds)} 个随机种子的实验已全部完成！")
    print(
        f"[{get_timestamp()}] Run complete. No experiment CSV files were written; figures, maps, and logs are in {base_output_dir}.")
